DEFINT A-Z
' EDIinput.bas  ( Edit Input )
'
' by: Jos‚ Luis Mares
'
' Edinput description:
' The Edinput let you edit any String that is already in memory
' or capture a new, and you can select the Row, Col and size of
' Max(Chars to capture). Also other features if you see the code.
' And keys Home, End, Delete, Left Arrow, Right Arrow, Up Arrow
' Down Arrow are enabled. Like Qbasic Input instruction.
'  (You can capture and edit several variables, see the example)
' usage:
' MyInput$ = EDInput(Row%, Col%, Max%, MyInput$)
' <MyInput$> inside of function is to kept the variable in memory
'  Example: Name$ = EDInput(4, 10, 30, Name$)
'  is like     Locate 4, 10: Input "Your Name :"; Name$
'
' You can delete all code that are marked : '** Can delete **
' to personalize the routine
'
' Tell me your opinion.....   jl_mares@hotmail.com
'
'Declaraci¢n de Subs y funciones
DECLARE FUNCTION EDInput$ (Ren%, Col%, Max%, Var$)
DIM SHARED DirCursor AS INTEGER

Example:    '** Can delete **
CLS '** Can delete **
LOCATE 10, 10: PRINT "Your name (15) ?"  '** Can delete **
LOCATE 12, 10: PRINT "Where age (2)  ?"  '** Can delete **
LOCATE 14, 10: PRINT "Your phone (15)?"  '** Can delete **
